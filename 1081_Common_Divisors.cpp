...
int ctz(int x) { return __builtin_ctzll(x); }
int clz(int x) { return __builtin_clzll(x); }
int ffs(int x) { return __builtin_ffsll(x); }
int bits(int x) {  // assert(x >= 0);
	return x == 0 ? 0 : 31 - __builtin_clz(x);
}  // floor(log2(x))
int p2(int x) { return 1ll << (x); }
int msk2(int x) { return p2(x) - 1; }
...
tcT > void mul(T &a, T b) { a%=MOD; b%=MOD; a *= b; if (a >= MOD) a -= MOD; }
...
void setIO(str s) {
	...
	if (fopen(FNAME".INP","r")) setIn(s + ".INP"), setOut(s + ".OUT");
}
int numMul[1000005];
int num[1000005];
void preprocess() {
	memset(numMul,0,sizeof(numMul));
...
void solve() {
	...
	fu(i,1,n)  { cin >> A[i]; num[A[i]]++; }
	preprocess();
	int res = 1;
	fu(i,1,1000000) {
		if (numMul[i]>1) ckmax(res,i);
	}
	cout<<res;
}
